...
export const useRoomStore = defineStore('rooms', {
  ...
  actions: {
    ...
    async addRoom(serverId, roomName) {
      try {
        const response = await axios.post(`http://localhost:8080/servers/${serverId}/rooms`, {
          name: roomName
        });
    ...
    async updateRoom({ serverId, roomId, newName }) {
      try {
        const response = await axios.put(`http://localhost:8080/servers/${serverId}/rooms`, {
          id: roomId,
          name: newName
        });
    ...
    async deleteRoom(serverId, roomId) {
      try {
        const response = await axios.delete(`http://localhost:8080/servers/${serverId}/rooms`, {
          data: {
